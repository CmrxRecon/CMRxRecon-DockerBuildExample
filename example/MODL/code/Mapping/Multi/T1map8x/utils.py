...
def complex_to_float32(complex_array):
    # 获取实部和虚部
    real = complex_array.real.astype(np.float32)
    imag = complex_array.imag.astype(np.float32)

    # 创建新的float32型数组
    float_array = real + imag*1j

    return float_array

def x_paired_paths_from_folder(folders, keys, filename_tmpl, mode):
    paths = []
    if mode == 'train':
        input_folder = folders[0]
        csm_folder = folders[3]
    else:
        input_folder = folders[2]
        csm_folder = folders[4]
    mask_folder = folders[1]   # gmv2
    input_key = keys[0]
    mask_key = keys[1]   # gmv2
    csm_key = keys[2]

    input_paths = list(mmcv.scandir(input_folder))  # 生成input文件里面图片的的list列表
    # input_paths = sorted(input_paths0)  # 对它们进行字典序排列，以获取按字母顺序排列的文件路径列表
    # csm_paths0 = list(mmcv.scandir(csm_folder))  # 生成input文件里面图片的的list列表
    # csm_paths = sorted(csm_paths0)  # 对它们进行字典序排列，以获取按字母顺序排列的文件路径列表

    for idx in range(len(input_paths)):
        input_path = osp.join(input_folder, input_paths[idx])
        input_prefix = input_paths[idx][0:10]  # 提取 input 文件的前缀 P***_cine_lax
        csm_filename = input_prefix + '_csm.mat'  # 构造 csm 文件名
        csm_path = osp.join(csm_folder, csm_filename)
        mask_path = osp.join(mask_folder, 'T1map_mask08.mat')   # gmv2,mask倍数选取
        paths.append(
            dict([(f'{input_key}_path', input_path),
                    (f'{mask_key}_path', mask_path),
                  (f'{csm_key}_path', csm_path)
                    ]))
    return paths
...
def c2r(complex_img, axis=0):
    ...
    if isinstance(complex_img, np.ndarray):
        real_img = np.stack((complex_img.real.astype(np.float32), complex_img.imag.astype(np.float32)), axis=axis)
