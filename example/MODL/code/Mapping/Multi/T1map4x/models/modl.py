...
class myAtA(nn.Module):
    ...
    def forward(self, im): #step for batch image
        ...
        """
        im_coil = self.csm * im.unsqueeze(1)  # split coil images (B x ncoil x nrow x ncol) [3, 10, 448, 204]
        k_full = torch.fft.fft2(im_coil, norm='ortho')  # convert into k-space
        k_u = k_full * torch.unsqueeze(self.mask, 1)  # undersampling [3, 10, 448, 204]
        im_u_coil = torch.fft.ifft2(k_u, norm='ortho')  # convert into image domain
        # im_u = torch.unsqueeze(torch.sum(im_u_coil * self.csm.conj(), axis=1), 1) # coil combine (B x nrow x ncol)
