...
class myAtA(nn.Module):
    ...
    def forward(self, im): #step for batch image
        ...
        """
        im_coil = im.contiguous() # split coil images (B x ncoil x nrow x ncol)
        ...
        im_u = im_u_coil # coil combine (B x nrow x ncol)
...
class data_consistency(nn.Module):
    def __init__(self):
        super().__init__()
        self.lam = nn.Parameter(torch.tensor(0.05), requires_grad=True)  # 将一个不可训练的tensor转换成可以训练的类型parameter，并将这个parameter绑定到这个module里面。即在定义网络时这个tensor就是一个可以训练的参数了。使用这个函数的目的也是想让某些变量在学习的过程中不断的修改其值以达到最优化。
...
class MoDL(nn.Module):
    ...
    def forward(self, x0, mask):
        ...
        for k in range(self.k_iters):
            ...
            #dc
            x_k = self.dc(z_k, x0, mask) # (2, nrow, ncol)
        return x_k
