...
def MaxMinNormalization(x):
    i_min = np.min(x)
    i_max = np.max(x)
    x1 = (x - i_min) / (i_max - i_min)
    return x1
def Fourier_2D(image_array1):
    img_c2 = np.fft.fft2(image_array1)
    img_c3 = np.fft.fftshift(img_c2)
    img_c4 = np.fft.ifftshift(img_c3)
    img_c5 = np.fft.ifft2(img_c4)
    return img_c5
def center_crop(image):
    slice_width, slice_height = image.shape
    width_crop = (slice_width - 128) // 2
    height_crop = (slice_height - 128) // 2
    if width_crop > 0:
...
def c2r(complex_img, axis=0):
    ...
    if isinstance(complex_img, np.ndarray):
        real_img = np.stack((complex_img.real, complex_img.imag), axis=axis).astype(np.float32)
...
def x_paired_paths_from_folder(folders, keys, filename_tmpl, mode):
    ...
    input_paths0 = list(mmcv.scandir(input_folder))  # 生成input文件里面图片的的list列表
...
def normalize(num):
    ...
    if type(num) == complex:
        ...
        real = (real / temp).astype(np.float64)
        imag = (imag / temp).astype(np.float64) * 1j
        return real + imag
    else:
        return num
