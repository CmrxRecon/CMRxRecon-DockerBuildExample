...
'''
path = '/media/nas/Seafile_Storage/Raw_data/MICCAIChallenge2023/ChallengeResult/Submission_MoDL/Multi_Mapping_04_T2/'

pattern = r'Multi_Mapping_(\d+)_([A-Za-z0-9]+)'
...
if Sub_Type == 'T1':
    pattern = r'mul_rec_P(\d+)_T1map_(\d+)_(\d+)\.mat'  # 文件名的正则表达式模式
elif Sub_Type == 'T2':
    pattern = r'mul_rec_P(\d+)_T2map_(\d+)_(\d+)\.mat'
else:
    print('SubType Error!')
...
# 存储每个人的数据到独立的文件夹中
savepath = '/media/nas/Seafile_Storage/Raw_data/MICCAIChallenge2023/ChallengeResult/Submission_MoDL/Mapping/MultiCoil/Mapping/ValidationSet/AccFactor'+ AccFactor
