...
'''
path = '/media/nas/Seafile_Storage/Raw_data/MICCAIChallenge2023/ChallengeResult/Submission_MoDL/Single_Cine_08_Lax/'

pattern = r'Single_Cine_(\d+)_([A-Za-z]+)'
...
if Sub_Type == 'Sax':
    pattern = r'single_rec_P(\d+)_cine_sax_(\d+)_(\d+)\.mat'  # 文件名的正则表达式模式
elif Sub_Type == 'Lax':
    pattern = r'single_rec_P(\d+)_cine_lax_(\d+)_(\d+)\.mat'
else:
    print('SubType Error!')
...
# 存储每个人的数据到独立的文件夹中
savepath = '/media/nas/Seafile_Storage/Raw_data/MICCAIChallenge2023/ChallengeResult/Submission_MoDL/Cine/SingleCoil/Cine/ValidationSet/AccFactor'+ AccFactor
...
for person, data_list in people_info.items():
    ...
    # 构建每个人的.mat文件完整路径
    if Sub_Type == 'Sax':
        file_path = os.path.join(person_folder, 'sax.mat')
    elif Sub_Type == 'Lax':
        file_path = os.path.join(person_folder, 'lax.mat')
    else:
        break
    # 将数据保存为.mat文件
